...
- **Error "Unsupported image" en identificación con IA (v2, robusto server-side)**: OpenAI rechazaba ciertas imágenes (formatos no aceptados, base64 con padding faltante, etc.). Solución definitiva: normalización server-side con Pillow en `game_identifier._normalize_image_to_jpeg_b64` que decodifica cualquier entrada (data URL, base64 crudo, PNG/JPEG/WEBP/HEIC/BMP/GIF), aplica EXIF, redimensiona a máx. 1280 px, y re-encoda como JPEG 82% antes de mandar a GPT-4o. Retry 1 vez ante errores transitorios. Errores clasificados: 400 (falta imagen), 422 (imagen inválida con mensaje en español), 502 (servicio IA caído). Verificado con 8/8 tests pytest (`/app/backend/tests/test_identify_game.py`).
